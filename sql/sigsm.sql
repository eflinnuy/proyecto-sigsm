...
CREATE TABLE categorias (id INT AUTO_INCREMENT PRIMARY KEY,nombre VARCHAR(100) NOT NULL UNIQUE);
...
CREATE TABLE servicios (id INT AUTO_INCREMENT PRIMARY KEY,nombre VARCHAR(100) NOT NULL UNIQUE);
...
INSERT INTO categorias(nombre) VALUES ('Enfermería'),('Nefrología y trasplante'),('Estudios'),('Prevención'),('Información general'),('Cardiología'),('Ginecobstetricia'),('Urología'),('Gastroenterología');
INSERT INTO servicios(nombre) VALUES ('Atención al paciente'),('Enfermería'),('Traslados'),('Información');
...
INSERT INTO traslados(paciente,chofer,enfermero,vehiculo,origen,destino,estado) VALUES ('Paciente de prueba','Juan Pérez','María Gómez','Ambulancia 01','Hospital de Clínicas','Clínica Central','En camino');
CREATE OR REPLACE VIEW resumen_encuestas AS SELECT c.id,c.nombre,COUNT(e.id) respuestas,ROUND(AVG(e.puntaje),2) promedio FROM encuestas_config c LEFT JOIN encuestas e ON e.encuesta_id=c.id GROUP BY c.id,c.nombre;
